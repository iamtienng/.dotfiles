...
`claude/` is a shared, all-platform package whose `.stowrc` targets **`~/.claude`** instead of `~/.config` (Claude Code reads config from `~/.claude`, not XDG). It holds `statusline-command.sh`, referenced by `~/.claude/settings.json`'s `statusLine.command`. `install.sh` stows it unconditionally (like `common/`) via the target-aware `stow_package "$pkg" "$CLAUDE_HOME"` / `prepare_config_entries "$CLAUDE_HOME" ...` helpers — these take the target dir as a parameter so the backup-existing logic works against `~/.claude` too. Note: `settings.json` itself is **not** stowed (it contains environment-specific endpoints/tokens); only files safe to share belong here.
